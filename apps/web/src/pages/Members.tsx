...
export default function Members() {
  ...
  useEffect(() => {
    ...
    // Map backend response to frontend Member type
    function mapToMember(m: MemberResponse): Member {
      const [firstName = "", lastName = ""] = (m.full_name || "").split(" ", 2);
      return {
        id: m.id,
        ...
        email: m.email,
        phone: null,
        status: "active", // Default; backend can add status later
    ...
    async function fetchMembers() {
      ...
      try {
        ...
        const data = await api.get<MemberResponse[]>(
          `/gyms/${DEFAULT_GYM_ID}/members`
        );

        if (!isMounted) return;

        // Map and deduplicate
        const mappedMembers = data.map(mapToMember);
        const uniqueMembers = Array.from(
          new Map(mappedMembers.map((m) => [m.id, m])).values()
        );
