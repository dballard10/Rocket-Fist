/**
 * @file Coach.tsx
 * @description Coach's class management page ("My Classes" view). Displays today's
 * classes assigned to the coach with a roster panel for managing attendance.
 * Primary interface for coaches to track and check in members during classes.
 *
 * @portal Staff
 * @roles coach (primary), owner (has access for oversight)
 * @route /my-classes (also /coach as legacy route)
 *
 * @features
 * - Left panel: Today's classes list with:
 *   - Class name, time, location
 *   - Reserved/checked-in counts
 *   - Visual selection indicator
 * - Right panel: Selected class roster with:
 *   - Member list with check-in status
 *   - "Mark Present" button for each reserved member
 *   - Real-time status updates
 *
 * @components
 * - TodayClassesList: List of today's classes for selection
 * - ClassRoster: Member roster with check-in controls
 *
 * @data
 * - Fetches from Supabase 'classes' and 'class_reservations' tables
 * - Falls back to mock data if Supabase unavailable
 *
 * @state
 * - classes: Today's class summaries
 * - selectedClassId: Currently selected class
 * - selectedClassRoster: Roster entries for selected class
 */
...
import type { ClassSummary, ClassRosterEntry } from "@rocket-fist/domain";
import { api, DEFAULT_GYM_ID } from "../lib/api/client";
...
const mockRosterData: Record<string, ClassRosterEntry[]> = {
  "1": [
    ...
    },
    {
      reservation_id: "r4",
      member_id: "m4",
      member_name: "Robin Taylor",
      status: "checked_in",
    },
    {
      reservation_id: "r5",
      member_id: "m5",
      member_name: "Casey Williams",
      status: "reserved",
    },
    {
      reservation_id: "r6",
    ...
    },
    {
      reservation_id: "r7",
      member_id: "m7",
      member_name: "Sam Johnson",
      status: "reserved",
    },
    {
      reservation_id: "r8",
      member_id: "m8",
      member_name: "Taylor Brown",
      status: "checked_in",
    },
    {
      reservation_id: "r9",
      member_id: "m9",
      member_name: "Jamie Garcia",
      status: "checked_in",
    },
    {
      reservation_id: "r10",
      member_id: "m10",
      member_name: "Drew Anderson",
      status: "checked_in",
    },
    {
      reservation_id: "r11",
      member_id: "m11",
      member_name: "Quinn Thomas",
      status: "checked_in",
    },
    {
      reservation_id: "r12",
      member_id: "m12",
      member_name: "Riley Jackson",
      status: "reserved",
    },
  ],
  "2": [
    ...
    },
    {
      reservation_id: "r25",
      member_id: "m13",
      member_name: "Avery White",
      status: "reserved",
    },
    {
      reservation_id: "r26",
      member_id: "m14",
      member_name: "Cameron Harris",
      status: "reserved",
    },
    {
      reservation_id: "r27",
      member_id: "m15",
      member_name: "Peyton Clark",
      status: "reserved",
    },
    {
      reservation_id: "r28",
      member_id: "m16",
      member_name: "Blake Lewis",
      status: "checked_in",
    },
    {
      reservation_id: "r29",
      member_id: "m17",
      member_name: "Skyler Robinson",
      status: "reserved",
    },
    {
      reservation_id: "r30",
      member_id: "m18",
      member_name: "Dakota Walker",
      status: "reserved",
    },
  ],
  "3": [
    ...
    },
    {
      reservation_id: "r40",
      member_id: "m19",
      member_name: "Jordan Mitchell",
      status: "reserved",
    },
    {
      reservation_id: "r41",
      member_id: "m20",
      member_name: "Sydney Moore",
      status: "reserved",
    },
    {
      reservation_id: "r42",
      member_id: "m21",
      member_name: "Parker Young",
      status: "reserved",
    },
    {
      reservation_id: "r43",
      member_id: "m22",
      member_name: "Hayden King",
      status: "reserved",
    },
    {
      reservation_id: "r44",
      member_id: "m23",
      member_name: "Emerson Scott",
      status: "reserved",
    },
    {
      reservation_id: "r45",
      member_id: "m24",
      member_name: "River Green",
      status: "reserved",
    },
  ],
  "4": [
...
export default function Coach() {
  ...
  useEffect(() => {
    // Backend schedule response type
    interface ScheduleClassInstance {
      id: string;
      class_id: string;
      gym_id: string;
    ...
    async function fetchTodaysClasses() {
      ...
      try {
        ...
        const data = await api.get<ScheduleClassInstance[]>(
          `/gyms/${DEFAULT_GYM_ID}/schedule?start=${dateStr}&end=${dateStr}`
        );
        ...
        // Map to ClassSummary (reservation counts would need a separate endpoint)
        const classSummaries: ClassSummary[] = data.map((c) => ({
          id: c.id,
          name: c.classes?.name || "Unnamed Class",
          start_time: c.start_time,
          end_time: c.end_time,
          location: "", // Backend doesn't have location yet
          reserved_count: 0, // Would need separate endpoint
          checked_in_count: 0,
        }));

        setClasses(classSummaries);
      } catch (err) {
        console.warn("Failed to fetch classes from API, using mock data:", err);
        setClasses(formattedMockClasses);
      } finally {
  ...
  // Note: Backend doesn't have a roster endpoint yet, using mock data
  useEffect(() => {
    if (!selectedClassId) {
    ...
    async function fetchRoster() {
      ...
      try {
        // TODO: Add roster endpoint to backend
        // For now, use mock data
        const mockRoster = mockRosterData[classId] || [];
  ...
  // Note: Backend doesn't have a check-in endpoint yet, updating locally only
  const handleMarkPresent = async (reservationId: string) => {
    setUpdatingReservationId(reservationId);

    try {
      // TODO: Add check-in endpoint to backend
